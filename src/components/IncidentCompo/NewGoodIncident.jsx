...
import GoodIRComp from './GoodIRComp';
import AntDesign from 'react-native-vector-icons/AntDesign';
import moment from 'moment';

const windowWidth = Dimensions.get('window').width;

const NewGoodIncident = ({incidenceReports}) => {
  const filterData = incidenceReports?.filter(item => item?.rate == 'Good');
  const itemsPerBox = 2; // Number of items per box
  const pageSize = 2; // Number of boxes per page
  const [currentPage, setCurrentPage] = useState(1);

  const boxes = filterData?.reduce((acc, item, index) => {
...
export default NewGoodIncident;
